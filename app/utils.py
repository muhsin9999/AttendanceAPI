...
def fetch_table_data(table, db, event_date, event, current_admin, columns_to_exclude=None):
    if columns_to_exclude:
        selected_columns = [getattr(table, column.name)
                            for column in table.__table__.columns if column.name not in columns_to_exclude]
    else:
        selected_columns = [getattr(table, column.name)
    ...
    column_names = [
        column.name for column in table.__table__.columns if column in selected_columns]
