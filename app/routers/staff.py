...
from app.services import manage_staff
...
async def create_staff(
    ...
    return manage_staff.create(request, db, current_admin)
...
async def show_all_staff(
    ...
    return manage_staff.show_all(db, current_admin)
...
async def show_staff(
    ...
    return manage_staff.show(id, db, current_admin)
...
async def remove_staff(
    ...
    return manage_staff.remove(id, db, current_admin)
...
async def update_staff(
    ...
    return manage_staff.update_st(id, updated_staff, db, current_admin)


@router.post("/images/{id}")
async def get_staff_image(
    id: int,
    action: schemas.StaffImageAction = Query(
        ..., description="Action to perform", example="upload"),
    files: Optional[List[UploadFile]] = File(..., min_items=5, max_items=10),
    db: Session = Depends(get_db),
    current_admin: dict = Depends(oauth2.get_current_admin)
):
    if action == schemas.StaffImageAction.upload:
        await manage_staff.upload(id, files, db, current_admin)
        return {"message": "Upload Successful"}
    elif action == schemas.StaffImageAction.capture:
        await manage_staff.capture(id, db, current_admin)
...
async def update_staff_image(
    ...
    await manage_staff.update_st_image(id, files, db, current_admin)
